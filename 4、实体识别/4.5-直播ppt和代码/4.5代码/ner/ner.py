...
from ner.models.crf.crfpp_predictor import CRFPPPredictor
...
class NER(object):
    ...
    def find(self, content, types=NER_TYPES, model='CRF', enable_offset=False, max_len=int(1e4)):
        ...
        # 规则干预
        self._rule_control(content, result)
        # 如果不返回索引，去重结果
    ...
    def _rule_control(self, content, result):
        """
        规则控制，可以借助一些正则
        ...
        for entity_type, rules in regex_list.items():
            for rule in rules:
                for match in re.finditer(rule, content):
                    result.get(entity_type, []).append((match.start(1), match.group(1)))  # 选择group1为我们想要的
