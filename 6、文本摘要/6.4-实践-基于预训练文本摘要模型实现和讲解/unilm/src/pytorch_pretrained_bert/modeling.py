...
class BertSelfAttention(nn.Module):
    def __init__(self, config):
        ...
        self.attention_head_size = int(
            config.hidden_size / config.num_attention_heads)  # 防止不是倍数
    ...
    def forward(self, hidden_states, attention_mask, history_states=None, mask_qkv=None, seg_ids=None):
        ...
        # 把QKV分割成num_heads份，把维度转换为[batch_size, num_heads, seq_length, all_head_size / num_heads]
        ...
        context_layer = torch.matmul(attention_probs,
                                     value_layer)  # 用注意力矩阵加权V，[batch, num_head, seq_len, all_head_size/num-head]
        context_layer = context_layer.permute(0, 2, 1, 3).contiguous()  # 输出tensor的shape与输入对齐
        new_context_layer_shape = context_layer.size()[:-2] + (self.all_head_size,)
        context_layer = context_layer.view(*new_context_layer_shape)  # [bs, seq_len, hidden_dim]
...
class BertAttention(nn.Module):
    def __init__(self, config):
        ...
        self.output = BertSelfOutput(config)  # dense -> layerNorm -> dropout
...
class BertIntermediate(nn.Module):
    def __init__(self, config):
        ...
        self.intermediate_act_fn = ACT2FN[config.hidden_act] if isinstance(config.hidden_act,
                                                                           str) else config.hidden_act
...
# multi-head attention（BertAttention-BertSelfAttention）
# -> 残差 & layer-norm （BertAttention-BertSelfOutput）
# -> feed-forward（BertIntermediate）
# -> 残差 & layer-norm (BertOutput)
class BertLayer(nn.Module):
    def __init__(self, config):
        super(BertLayer, self).__init__()
        self.attention = BertAttention(config)  # multi-head attention
        self.ffn_type = config.ffn_type
        if self.ffn_type:
...
class BertEncoder(nn.Module):
    def __init__(self, config):
        ...
        self.layer = nn.ModuleList([copy.deepcopy(layer)
                                    for _ in range(config.num_hidden_layers)])  # n层BertLayer
...
class PreTrainedBertModel(nn.Module):
    ...
    def from_pretrained(cls, state_dict=None, *inputs, **kwargs):
        ...
        # define new type_vocab_size (there might be different numbers of segment ids),更新配置
...
class BertModel(PreTrainedBertModel):
    ...
    def get_extended_attention_mask(self, input_ids, token_type_ids, attention_mask):
        ...
        extended_attention_mask = (1.0 - extended_attention_mask) * -100000.0  # 0 变为-10000->softmax
        return extended_attention_mask

    def forward(self, input_ids, token_type_ids=None, attention_mask=None, output_all_encoded_layers=True,
                mask_qkv=None, task_idx=None):
        extended_attention_mask = self.get_extended_attention_mask(
            input_ids, token_type_ids, attention_mask)  # 维度扩充下面使用
        ...
        sequence_output = encoded_layers[-1]  # 取出最后一层的隐向量
        pooled_output = self.pooler(sequence_output)  # 计算Pooler结果
        if not output_all_encoded_layers:
...
class BertForPreTrainingLossMask(PreTrainedBertModel):
    ...
    def __init__(self, config, num_labels=2, num_rel=0, num_sentlvl_labels=0, no_nsp=False):
        ...
        self.bert = BertModel(config)  # transformer-encoder
        self.cls = BertPreTrainingHeads(  # MLM + NSP
        ...
        self.crit_mask_lm = nn.CrossEntropyLoss(reduction='none')  # MLM的loss-function, 直接返回所有样本的loss
        if no_nsp:
            self.crit_next_sent = None
        else:
            self.crit_next_sent = nn.CrossEntropyLoss(ignore_index=-1, reduction='none')  # NSP的loss-function
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, masked_lm_labels=None,
        ...
        # sequence_output: [bs, seq_len, hidden_dim], pooled_output:[bs, hidden_dim]
        ...
        # 把mask位置的隐向量对应取出来，masked_pos：mask的index位置
        sequence_output_masked = gather_seq_out_by_pos(sequence_output, masked_pos)

        # prediction_scores_masked:计算在字典上的预测概率分布; seq_relationship_score:NSP的概率
        prediction_scores_masked, seq_relationship_score = self.cls(sequence_output_masked,
                                                                    pooled_output,
                                                                    task_idx=task_idx)
        if self.crit_mask_lm_smoothed:  # --
            masked_lm_loss = self.crit_mask_lm_smoothed(
                F.log_softmax(prediction_scores_masked.float(), dim=-1), masked_lm_labels)  # [bs, max_mask_len]
        else:
            masked_lm_loss = self.crit_mask_lm(
                prediction_scores_masked.transpose(1, 2).float(), masked_lm_labels)

        masked_lm_loss = loss_mask_and_normalize(masked_lm_loss.float(), masked_weights)  # masked_weights:mask的mask

        # next sentence
        if self.crit_next_sent is None or next_sentence_label is None:
            next_sentence_loss = torch.tensor(0.0).type_as(masked_lm_loss)
        else:
            next_sentence_loss = self.crit_next_sent(seq_relationship_score,
                                                     next_sentence_label)  # [bs, ], ignore_index=-1（只MLM有次项）
            # 是按照一定的规则合并两个tensor类型。task_idx!=0, 则对应位置为0, task_idx==0, 对应位置为1
            next_sentence_mask = torch.where(task_idx != 0,
                                             torch.full_like(task_idx, 0),
                                             torch.full_like(task_idx, 1)).type_as(next_sentence_loss)
...
class BertForSeq2SeqDecoder(PreTrainedBertModel):
    ...
    def get_prediction_scores(self, input_ids, token_type_ids, position_ids, attention_mask, decode_mask, task_idx=None,
                              mask_qkv=None):
    ...
    def forward(self, input_ids, token_type_ids, position_ids, attention_mask, decode_mask, task_idx=None,
                mask_qkv=None):
        if self.rule != 3:
            return self.get_prediction_scores(input_ids, token_type_ids, position_ids, attention_mask, decode_mask,
                                              task_idx=None, mask_qkv=None)
        if self.search_beam_size > 1:
            return self.beam_search(input_ids, token_type_ids, position_ids, attention_mask, decode_mask,
    ...
    def beam_search(self, input_ids, token_type_ids, position_ids, attention_mask, decode_mask, task_idx=None,
                    mask_qkv=None):
        input_shape = list(input_ids.size())
        batch_size = input_shape[0]
        input_length = input_shape[1]
