...
class BeamSearch(object):
    def __init__(self, model_file_path):
        dt_batch = datetime.now().strftime("%Y-%m-%d_%H%M%S")
        model_name = os.path.basename(model_file_path)
        self._test_dir = os.path.join(config.log_root, 'decode')
        self.pre_res_dir = os.path.join(self._test_dir, 'pre_res_%s_%s.csv' % (model_name, dt_batch))
        ...
        self.batcher = Batcher(data_path=config.decode_data_path, vocab=self.vocab, mode='decode',
                               ...
                               max_iterations=config.max_iterations, is_test=True)
        # time.sleep(15)

        self.model = Model(model_file_path, is_eval=True, is_tran=False)
    ...
    def beam_search(self, batch):
        ...
        dec_h, dec_c = s_t
        dec_h = dec_h.squeeze()     # (bs, 2×hidden_dim)
        dec_c = dec_c.squeeze()     # (bs, 2×hidden_dim)
        ...
        while steps < config.max_dec_steps and len(results) < config.beam_size:
            ...
            latest_tokens = [t if t < self.vocab.size() else self.vocab.word2id(config.UNK_TOKEN) \
                             for t in latest_tokens]    # 上一时刻解码的token
            ...
            c_t = torch.stack(all_context, 0)   # (b_s, 2*hidden_dim)
    ...
    def run(self):
        ...
        while batch is not None:
            ...
            result.append([batch.original_articles[0], original_abstract_sents[0], ''.join(decoded_words)])
        ...
        result = pd.DataFrame(result, columns=['raw_title', 'refs', 'pres'])
        result.to_csv(self.pre_res_dir, index=False, header=False)
...
if __name__ == '__main__':
    model_filename = '/home/humeng/e/projects/hot_search/modules/pointer-generator-master/logs/train_1611738819/models/model_17000'
