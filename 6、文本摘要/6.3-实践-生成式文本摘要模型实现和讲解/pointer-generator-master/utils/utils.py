...
def article2ids(article_words, vocab):
    ...
    输入article中所有token的id, 如果在词典中就是词典中对应的id,如果是OOV,就是当前batch扩展的id(扩展只是在当前batch中进行),
    如只有1个:id=vocab_size,2个:id=vocab_size+1
    :param article_words:
    :param vocab:
    :return:
    """
    ...
    for w in article_words:
        ...
        if i == unk_id:  # If w is OOV
            ...
            ids.append(
                vocab.size() + oov_num)  # This is e.g. 4000 for the first article OOV, 4000 for the second...,从原始vocab中扩展下标
        else:
            ids.append(i)
    return ids, oov
...
def abstract2ids(abstract_words, vocab, article_oovs):
    ...
    对摘要的id转换
    如果是字典中则转为id,
    如果是文章中的oov,则使用文章中oov下标扩展id来作为临时id
    如果都不是,使用unk_id
...
def get_input_from_batch(batch, use_cuda):
    """
    1. 取出encoder段需要数据,tensor表示
    2. 放如到cuda中
    :param batch:
    :param use_cuda:
    :return:
        enc_batch: article的在vocab原始id,(UNK = 0,PAD = 1)
        enc_lens: batch中每个article的句子长度
        enc_pos: article中token的位置矩阵
        enc_padding_mask: mask矩阵
        enc_batch_extend_vocab: 输入article中所有token的id(使用OOV扩展过的)
        extra_zeros:  本batch一共有多个少OOV,就需要对应扩展多少0占位向量, shape:[bs, sum_oov_num]
        c_t: 初始化上下文向量, [bs, 2 * hidden_dim]
        coverage: 初始化coverage向量, [bs, max_enc_len]
    """
    extra_zeros = None
    max_enc_len = np.max(batch.enc_lens)
    # enc_lens = torch.tensor(batch.enc_lens, dtype=torch.long)
    ...
    if config.pointer_gen:
        ...
        if batch.max_art_oovs > 0:
            extra_zeros = Variable(torch.zeros((batch_size, batch.max_art_oovs)))  # 根据oovs最大数量,扩充向量
